...
4. 입력
   ...
   | **背景画像** | **선택.** 그 분류의 최신 글이면 맨 위 큰 카드에 배경으로 깔린다. 없어도 안 깨진다 |
...
3분류(韓国·台湾·お知らせ)를 통틀어 **가장 최신 글 1건**이 INSIGHT 맨 위에 크게 선다.
따로 지정하는 게 아니라 **날짜순으로 자동**이다.

- 배경 이미지를 넣으면 사진 위에 제목이 얹힌다
- **안 넣어도 된다.** 검정 판 그대로 나오고, 그것도 완성된 모습이다
- ⚠️ 이미지를 만들어야 발행할 수 있으면 그게 병목이 된다. 좋은 사진이 있을 때만 넣는다
...
- 기본은 **왼쪽 정렬**. 문장이 들어가는 표는 그대로 두면 된다
- 숫자 열을 오른쪽에 붙이려면 구분선에 `:` 를 쓴다
